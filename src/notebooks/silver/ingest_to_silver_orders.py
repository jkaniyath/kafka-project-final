...
1. SilverOrdersIngestion: Likely responsible for ingesting orders data into the silver layer of the data pipeline.
...
from python_package.silver.silver_orders_ingestion import SilverOrdersIngestion
...
1. Initialize SilverOrdersIngestion: Creates an instance to handle the ingestion of orders data, using the `spark` session, a checkpoint path, and environment details.
2. Call silver_orders_ingestion Method: Starts the ingestion process by calling the method to process the orders data.
...
try:
    orders_ingestion = SilverOrdersIngestion(spark=spark, base_checkpoint_path=checkpoint_location, env=env)
    orders_writter = orders_ingestion.silver_orders_ingestion(processing_time="30 seconds")
